...
**Phase-19 integration status (keep current — the test/integrated boundary).**
- **UNDER TEST — lives only on `feature/exogenous-engine`, does NOT affect main's pipeline:** the whole exogenous engine (`exogenous/` — `spf`, `sep`, `fed_docs`, `extract`, `analyst`, `synth`, `run_slice`, `emit`) and its A/B hooks in `score_predictions.py` (arm-keyed score files) + `summarize_accuracy.py` (`calibration_by_arm`). None of this is on `main`.
- **On `main` (inert triggers only):** `exo_slice_smoke.yml` and `exo_weekly_emit.yml` as **dispatch-only** copies — they exist so the Actions "Run workflow" button appears; neither auto-runs (no active `schedule:`). Run them manually **against `feature/exogenous-engine`** so all test artifacts (exogenous notes, scores) stay on that branch.
- **Test loop (fully on the feature branch):** dispatch *Exogenous Weekly Emission* → note in feature-branch `results/`; ~1 wk later dispatch *Weekly Prediction Scoring* against the feature branch → arm A/B in that branch's `accuracy_report.md`. Main's scheduled jobs read main's data, untouched.
- **INTEGRATION (only after the test phase clears the DESIGN §5 bar → KB-012):** merge `feature/exogenous-engine` → `main`, then **uncomment the `schedule:` block** in `exo_weekly_emit.yml` to turn on the weekly cadence. That merge is the single, deliberate integration step.