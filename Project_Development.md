...
2. **WP-18.2 — Cheap input-quality proxies (zero-cost, no LLM).** Per input section/series, compute and log: **staleness** (already surfaced for FRED — e.g. gdp 173d, cpi 53d; a stale series is low-value), **variance/entropy** (a near-constant input carries no information), **cross-input redundancy** (collinearity / mutual information — 10y vs 2y vs real-yield vs breakeven almost certainly move together; SPY vs the 11 sector ETFs; quant subsections vs their FRED inputs), and **token cost**. Output: an "input ledger" ranking sections/series by information density and flagging the stale / redundant / dead payload. This **screens** candidates for the expensive ablation — it does not by itself prove value.

3. **WP-18.3 — Model-attention / citation screen (low-cost).** Parse each prediction's rationale (Primary Driver, Key Risks) for references to each input and track per-input citation frequency over time; an input the model never references is a prune candidate. **Caveat (KB-discipline):** citation is a *screening* proxy, never a verdict — the model can use an input without naming it, or name it without it mattering. Flags candidates for 18.4; does not decide.
