...
2. **WP-19.B — One vertical slice (L0→L4 for a single branch).** Recommended first branch: **monetary / rates-expectations** — free text, genuine expectations-gap tradability, and it maps onto assets we already forecast (10Y, DXY, gold). Prove it produces a signal that tracks surprises / moves Brier. *If this one slice cannot clear the bar, the concept is weak and we have spent very little — that is the entire point of going narrow first.* **Progress (2026-07-14):** L0 **SPF adapter built** — `.macro-assist/exogenous/spf.py` (pure parser + release-date mapping + point-in-time `latest_before` + cross-variable `snapshot_from_series`; thin `.xlsx` loader needs openpyxl, added to requirements). 15 tests, no I/O. Parser is verified against the documented SPF schema via synthetic frames; the **real-file layout (exact horizon-column→quarter mapping, sheet name, .xls vs .xlsx) needs one confirmation on first download** (HORIZON_NOTE in the module). Next: FRED **SEP** pull (existing adapter) + the SPF-vs-SEP gap, then the FOMC-text L1 extractor / L2 analyst LLM layers.
