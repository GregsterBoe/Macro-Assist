...
| Daily run log (`macro_daily.yml`, last step) | `[FEED]` one-liners from `feed_audit.py` — the live degraded streak, the latest reading's recorded cause, and whether it is within tolerance. `FAIL` and a non-zero exit past the threshold, which turns the day's run red *after* the note has been published |
