...
- **Latest:** **shipped 2026-09-04.** `numeric_baseline.py` carries two new arms — `exogenous_spf` (SPF features only, no price or market input) and `market_plus_exo` (the market panel plus those columns, read against `ridge`). Seven features off the committed SPF workbooks: consensus curve, consensus path at 4 quarters, three survey-to-survey revisions, a staleness clock. Zero LLM spend, no new secret, no new network dependency. 16 new tests; full suite green.
