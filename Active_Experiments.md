...
- **Shipped:** **WP-21.A.1 ✅ (harness only)** — `numeric_baseline.py`: `strategy_ridge` / `strategy_gbm` against the existing `backtest.py` interface, walk-forward with a **`horizon + 1` embargo**, scored by the *production* readers (`score_call`, `_brier_and_reliability`, `bias_separation`) against `neutral` / `random_walk` / `always_bullish` on the same dates. Point-in-time without ALFRED's 40k calls: **only never-revised inputs are eligible** (prices + daily market-observed FRED), enforced by a test, each shifted one business day. Emits per-input weights, sign stability and out-of-sample permutation drops; `ret_20` carries the 20-day reversion question. Output isolated to `results/numeric_baseline/`. Pre-committed edge bar in `verdict()`. 33 tests, incl. a **planted-signal positive control** so a "no edge" reading can't just be a dead harness. 615 green.
- **Ran:** **WP-21.A first read (2026-09-03, `output` branch)** — 18y, 75,414 model calls, 3,601 refits. **No edge, on the wrong side of the trivial benchmark.** `ridge` 0.530 / BSS −0.087 and `gbm` 0.526 / BSS −0.059 both sit *below* `always_bullish` (0.560, BSS −0.000) on hit-rate, Brier, BSS and ECE; both invert on separation (bear−bull +0.093, p=0.002, bootstrap CI [0.021, 0.165], strengthening t5→t20). Mechanism is legible and shared by both model classes: `drawdown` / `rv_20` / `vol_ratio` are the only inputs with positive out-of-sample permutation drop, all signed *stress → bearish*, and stress mean-reverts at 10–20d — hence the 90–100% confidence bin hitting **0.404**. `ret_20` is dead as a standalone feature (sign stability 0.778, the lowest of 20; permutation −0.000), which closes the 20-day reversion errand negatively.
- **Shipped:** **WP-21.A.2 ✅ (sample-alignment fix)** — the first run committed the [KB-023] error one level down: `comparator_calls` keyed off *price availability*, so the comparators got 78,656 calls to the models' 75,414. The gap was Bitcoin (9,331 vs 6,272 — the models need 756 days of BTC history the comparator did not) plus WTI (183), i.e. free extra sample for `always_bullish`, the benchmark the whole verdict turns on. Now `shared_call_keys` intersects the (window, date, asset) triples across model arms and `restrict_calls` clamps *every* arm to them; the report prints the shared sample size and a ⛔ if the arms ever diverge again. **The direction of the result does not change** — even if all 3,242 extra calls were decisive hits, `always_bullish` only falls to ≈0.537, still above `ridge`'s 0.530 — but the headline number is provisional until the re-run. 592 green.
- **Next:** **re-run WP-21.A on the aligned sample**, then write the KB entry from `results/numeric_baseline/numeric_baseline.md`. Quote `bearish_vs_bullish` (p=0.002, CI excludes 0) for the inversion, **not** `bullish_vs_neutral` (p=0.004 but CI [−0.094, +0.015] spans zero — `bias_separation`'s own docstring says the interval is what separates underpowered from null). Also pull `scores.json.gz` off the 30-day CI artifact before ~2026-10-03 if per-asset hit-rate/BSS is wanted; it is stripped before publish. That entry decides WP-21.C and WP-21.D. Then **WP-21.B.2** — day-alternating profile assignment.
- **Where:** `Project_Development.md` (Phase 21) · `.macro-assist/numeric_baseline.py` · `.macro-assist/backtest.py` · branch `claude/latest-changes-foundational-tests-y5qqxu`.
