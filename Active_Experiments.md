...
- **Next:** **run WP-21.A** — needs `FRED_API_KEY` + reachable yfinance/FRED; the build session had neither, so **there is no number yet and no KB entry.** `python .macro-assist/numeric_baseline.py --start 2005-01-01 --save-panel panel.csv`, then write the KB entry from `results/numeric_baseline/numeric_baseline.md`. That entry decides WP-21.C and WP-21.D. Then **WP-21.B.2** — day-alternating profile assignment.
