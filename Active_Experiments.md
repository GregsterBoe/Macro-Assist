...
What is left running is the fragility track and the numeric/quant work — into which
the Phase-19 exogenous anchor has now been folded as two arms (WP-19.E)._
...
### WP-19.E — The exogenous anchor, scored in the numeric harness 🟡 (built; the run is open)
...
- **Latest:** **the 2026-09-05 run is VOID — the exogenous arms never ran [KB-025].** The workflow built the flag as `${{ inputs.exogenous && '' || '--no-exogenous' }}`, an idiom that returns the fallback whenever its truthy branch is the empty string, so `--no-exogenous` was passed on *every* dispatch. The run was green for 2h50m and published a valid **market-only** report (`n_exo_features: 0`) under a WP-19.E heading; the only tell was one metadata line. Fixed 2026-09-07: the workflow branches in the shell, `--require-exogenous` makes a fallback a hard failure, and six tests in §9 assert both. What shipped 2026-09-04 is unchanged and still correct — two arms, `exogenous_spf` (SPF features only, no price or market input) and `market_plus_exo` (the market panel plus those columns, read against `ridge`); seven features off the committed SPF workbooks: consensus curve, consensus path at 4 quarters, three survey-to-survey revisions, a staleness clock. Zero LLM spend, no new secret, no new network dependency. Full suite green (661).
...
- **Next:** **re-run** `Numeric Directional Baseline` from the Actions tab (`exogenous: true`, the default) on the branch carrying the [KB-025] fix — ~3h, needs `FRED_API_KEY` + network, which is why it is a CI job. **Check the log line `exogenous input: 'true' -> --require-exogenous` and the report's `n_exo_features` before reading a single number**; a run that could not carry the anchor now fails rather than falling back. **The read is pre-registered** (Phase 19 → WP-19.E) and unspent: the anchor arm must clear the standing bar *and* beat `always_bullish`; the increment counts only if `market_plus_exo` clears the bar in absolute terms *and* improves BSS over `ridge`. Then the KB entry. Blocks nothing.
- **Where:** `Project_Development.md` (Phase 19 → WP-19.E) · `.macro-assist/numeric_baseline.py` · `.github/workflows/numeric_baseline.yml`.
...
- **Next:** the branch's deterministic half is now under test as **WP-19.E** above. What remains for the branch proper is **(b)**: re-cut its output the way the main note was re-cut. DESIGN §1 says it is about expectations-gaps and regime **not** direction; the directional lean was a scoring convenience and is exactly the part that became unscoreable. Publishing the expectations gap itself is the coherent product — and it is the only route that reaches the two-layer mechanism WP-19.E cannot test.
