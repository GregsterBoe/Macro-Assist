...
| **Watch** | The Fragility Monitor's `vix_term` leg was missing 2026-09-16 → 09-18 — no calibrated label on those three readings ([KB-034]). **Cause found:** yfinance's `^VIX3M` returns nothing at ~06:04 UTC (when the note runs) and current data at 16:24 UTC the same day. Shipped: instrumentation, a daily gate (`feed_audit.py`, pipeline job `feed_gate`, red past two consecutive degraded readings without blocking the weekly stages) and `pipeline.yml mode=validate`. **2026-09-22:** the legs now retry (`pipeline_common.yf_history_with_retry` — an *empty frame* is a failure, not an answer), and `--probe-cboe` runs in CI as its own job `feed_probe`. The probe is **green off the runner** and failed 3/3 on it, so the code path is sound and the difference is environmental ([KB-034] addendum); **until CI reports, still treat the vol legs as having no fallback.** The cause itself is untouched — the feed is empty at 06:04 and current at 16:24, so *moving the run* (`todo.md` #26 item 3) is the open decision, and it shares a crontab with #27. |
| **Red on purpose** | `record_audit.py` is red on `output:schedule/last-cron-catchup.txt`. The catch-up call had never once fired across 39 runs while `operations.md`, `pipeline.yml`'s header and [ADR-0012](docs/decisions/ADR-0012-external-cron-with-backstop.md) all rested on it — invisible, because a slot that never ran and a slot that ran and no-op'd both write nothing. Every run now leaves `schedule/last-<source>.txt` (job `heartbeat`) and all three slots are `ARTIFACTS` entries. **The red clears when the second call is added to the caller** — cron-job.org, so a second HTTP job, not a crontab line (`operations.md` → *On an HTTP-only service*) — it is the item, not a stale entry, and must not be pinned. `todo.md` #27 |
| **Winding down** | The directional scorer, once the last T+20 window resolves ~2026-10-02 |
| **Exploring** | Phase 23 — harness built, three looks run 2026-09-14 and 09-21 on 2010–2017; register holds H-001 `closed` (confound resolved, no KB entry), H-002–H-004 `draft`, H-005–H-007 `seen`. Seal decided: `SEAL_START` 2018-01-01 reused for the distribution class (`resolved.md` #19); `har_scaled` is a comparator in WP-23.B's bar, not the scorer's (#22). Nothing open in the inbox for this phase; next is the owner's rewrites, then the class bar |
