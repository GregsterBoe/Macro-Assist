...
def build_note(
    ...
    #
    # v1.6: the per-lever keys below are kept because the historical readers parse
    # them off every note, but the levers themselves are INERT — every prompt block
    # they gated was a directional-call rule and those are gone (WP-21.D). The
    # human-readable `config:` line says so rather than advertising a floor that
    # floors nothing.
    _rc = run_config()
...
"""
    ...
    # Fragility Monitor (Phase 16) — computed post-analysis from the logged reading.
    #
    # WP-21.D / v1.6: PROMOTED. It used to be appended to the tail of the Data
    # Snapshot, below the FRED table, because the note's headline was the
    # directional call. [KB-024] cut that call, and the fragility flag is the
    # product that survived honest out-of-sample evaluation, so it now leads the
    # note — directly under the title, above the analysis.
    fragility_section = ""
    if quant_raw:
        try:
            ...
            if _frag_block:
                fragility_section = _frag_block.rstrip("\n") + "\n\n---\n"
        except Exception:
            fragility_section = ""
